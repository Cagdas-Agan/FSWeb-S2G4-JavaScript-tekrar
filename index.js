...
tekraredensayilar = [];
const sayiFrekanslari = {};

// Frekansları hesaplama
sayilar.forEach((sayi) => {
  sayiFrekanslari[sayi] = (sayiFrekanslari[sayi] || 0) + 1;
});

// Tekrar eden sayıları bulma ve diziye ekleme
for (const [sayi, frekans] of Object.entries(sayiFrekanslari)) {
  if (frekans > 1) {
    tekraredensayilar.push(`${sayi} sayısı ${frekans} kere tekrar edilmiştir`);
  }
}
